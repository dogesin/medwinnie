...
export function getHistoryChatMessages(caseId: string): Promise<ChatMessage[]> {
  return apiFetch<ChatMessage[]>(`/cases/${caseId}/questions`)
}

export function getHistorySuggestedQuestions(caseId: string): Promise<string[]> {
  return apiFetch<string[]>(`/cases/${caseId}/questions/suggestions`)
}

export function sendHistoryQuestion(
  caseId: string,
  questionText: string
): Promise<ChatMessage> {
  return apiFetch<ChatMessage>(`/cases/${caseId}/questions`, {
